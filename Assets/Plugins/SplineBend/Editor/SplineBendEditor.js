...
class SplineBendEditor extends Editor 
{
	...
	function OnInspectorGUI()
	{
		...
		for (var m:int=0; m<target.markers.length; m++)
	    {
	        if (!target.markers[m]) 
		    {
	             target.UpdateNow(); 
        ...
        if (GUILayout.Button("+")) 
        {
            ...
            markers = new SplineBendMarker[target.markers.length+1];
            for (var n:int=0; n<target.markers.length; n++)
            {
                markers[n] = target.markers[n];
            }
            var obj = GameObject.Instantiate(markers[markers.length - 2].gameObject);
            obj.transform.parent = markers[markers.length - 2].transform.parent;
            obj.transform.localPosition = markers[markers.length - 2].transform.localPosition;
            obj.name = "Marker" + (markers.length-1).ToString();
            markers[markers.length - 1] = obj.GetComponent("SplineBendMarker");
